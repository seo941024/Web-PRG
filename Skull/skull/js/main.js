...
function updatePlayer() {
    ...
    if (dn("KeyZ") && p.dashCD <= 0 && !p.guarding && p.kbT <= 0 && p.atkT === 0) {
        if (typeof consumeStamina === 'function' && !consumeStamina(STAMINA_DASH || 28)) {
            // 스태미나 부족 시 짧은 Fat Roll
            p.dashT = p.fatRoll ? 6 : 15 + Game.pDashInv;
        } else {
...
function updatePlayerCombat() {
    ...
    if (dn("KeyC") && !dn("ArrowDown") && p.atkT === 0 && !p.guarding && p.kbT <= 0 && p.dashT <= 0 && !p.plunging) {
        if (typeof consumeStamina === 'function') consumeStamina((typeof STAMINA_ATK !== 'undefined') ? STAMINA_ATK : 10);
        playSfx('atk');
    ...
    if (dn("ShiftLeft", "ShiftRight") && Game.pMp >= 100 && !p.guarding && p.kbT <= 0 && p.dashT <= 0 && !p.plunging && p.atkT === 0) {
        Game.pMp -= 100;
        p.atkT = 25;
        if (typeof consumeStamina === 'function') consumeStamina((typeof STAMINA_SKILL !== 'undefined') ? STAMINA_SKILL : 40);
            ...
            if (near && dn("ArrowUp") && !K._evOld) {
                ...
                if (ev.type === "curse_altar") {
                    ...
                    if (roll < 0.33) {
                        ...
                        addText(ev.x, ev.y - 20, "저주: 최종 데미지 +50%!", "#ff0055", 120, 13);
                    } else if (roll < 0.66) {
                        ...
                        Game.pCritDmg += 1.0;
                        addText(ev.x, ev.y - 20, "저주: 치명타 +25% / +100%!", "#ff0055", 120, 13);
                    }
                    ...
                    // 화톳불: HP/MP/스태미나 완전 회복, 몬스터 부활
                    K._evOld = true;
                    if (typeof useBonfire === 'function') useBonfire(ev);
                    ev.used = true; // 이미 used=true 처리
...
function update() {
    ...
    if (typeof updateRegain === 'function') updateRegain();
...
function startGame() {
    ...
    // 영구 강화 신규 스탯 반영
    Game.pMoveSpdMul  = 1.0 + (Game.permSpdLvl  || 0) * 0.04;
    Game.pJmpMul      = 1.0 + (Game.permJmpLvl  || 0) * 0.05;
    Game.pDashCDMul   = 1.0 - Math.min(0.5, (Game.permDashLvl || 0) * 0.05);
    Game.pCritDmg     = 1.5 + (Game.permCritDmgLvl || 0) * 0.15;
    Game.pMaxMp       = 100 + (Game.permMpLvl || 0) * 15;
...
function loop(currentTime) {
    ...
    switch (Game.gs) {
        ...
        case "opening_anim":
            ...
            break;
        case "opening_end":
            // 조립 완료 대기 - renderOpeningAnim이 waitPhase 상태로 처리
            if (typeof updateOpeningAnim === 'function') updateOpeningAnim();
            if (typeof renderOpeningAnim === 'function') renderOpeningAnim(frameNow);
            break;
